...
def _get_js_dependency_urls(enable_histogram):
    ...
    js_dependency_urls = [
        ...
        "https://d3js.org/d3.v7.min.js",  # Add D3.js
    ]
...
def render_html(
    ...
    nodes = []
    for _, row in point_dataframe.iterrows():
        nodes.append(
            {
                "id": row.name,
                "x": row["x"],
                "y": row["y"],
                "cluster": row["cluster"],  # Assuming 'cluster' column exists
                "color": f"rgb({row['r']},{row['g']},{row['b']})",
            }
        )

    # Create links (you may need to adjust this based on your data structure)
    links = []
    ...
    force_graph_data = {"nodes": nodes, "links": links}
    template_vars = {}
    # Add the force graph data to the template context
    template_vars["force_graph_data"] = json.dumps(force_graph_data)
